...
import * as brandApi from "../../endpoints/brand";
...
const CreateProduct = () => {
  ...
  const { data: colors } = useQuery({
    queryKey: ['colors'],
    queryFn: categoryApi.getColors,
  });

  const { data: currencies } = useQuery({
    queryKey: ['currencies'],
    queryFn: categoryApi.getCurrency,
  });
        ...
        <form className={style.form} onSubmit={(e) => e.preventDefault()}>
          ...
          <div className={style.form_section}>
            ...
            >
              ...
              {colors?.map((color: { id: number | string; name: string }) => (
          ...
          {/* Currency Selection */}
          <div className={style.form_section}>
            <label className={style.label}>Valyuta *</label>
            <select
              className={style.select}
              value={productData.currency_id || ""}
              onChange={(e) =>
                setProductData({ ...productData, currency_id: +e.target.value })
              }
              required
            >
              <option disabled value="">
