...
export class SmsService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!this.token) {
      this.logger.warn('SMS_TOKEN not found in environment variables - SMS service will run in development mode');
  ...
  async sendSMS(phoneNumber: string, message: string): Promise<string> {
    try {
      ...
      if (!this.token) {
        this.logger.warn(`Development mode: SMS simulated for ${formattedPhone}: ${message}`);
