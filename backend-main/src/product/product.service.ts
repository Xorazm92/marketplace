...
export class ProductService {
  ...
  async create(createProductDto: CreateProductDto, userId?: number) {
    const { images, user_id, product_color_ids, ...productData } = createProductDto;
    ...
    // Link colors if provided
    if (product_color_ids && Array.isArray(product_color_ids) && product_color_ids.length > 0) {
      try {
        await this.prisma.productColor.createMany({
          data: product_color_ids.map((colorId) => ({
            product_id: product.id,
            color_id: Number(colorId),
          })),
        });
      } catch (e) {
        console.warn('Failed to link product colors:', e?.message || e);
      }
    }
  ...
  async getAllProduct(category?: string) {
    ...
    if (category) {
      // Support both category name and slug filtering
      where.OR = [
    ...
    const products = await this.prisma.product.findMany({
      ...
      include: {
        product_image: {
          orderBy: { id: 'asc' } // Ensure consistent image ordering
        },
        brand: true,
        category: true,
        user: {
    ...
    return products.map(product => {
      ...
      const images = product.product_image?.map(img => {
        ...
        // Otherwise, assume it's just filename and add /uploads/ prefix
  ...
  async setPrimaryImage(productId: number, imageId: number) {
    try {
      ...
      const updatedImage = await this.prisma.productImage.update({
        where: { 
          id: imageId,
          product_id: productId // Ensure the image belongs to the product
        },
