...
export class LoadBalancerService {
  ...
  async checkRateLimit(
    ...
    try {
      const current = await this.redisService.incr(currentKey);
      
      if (current === 1) {
        await this.redisService.expire(currentKey, Math.ceil(config.windowMs / 1000));
      }
  ...
  private roundRobin(servers: string[]): string {
    const key = 'load_balancer:round_robin';
    const index = parseInt((this.redisService.get(key) || '0')) % servers.length;
    this.redisService.setex(key, 3600, ((index + 1) % servers.length).toString());
    return servers[index];
  }

  private leastConnections(servers: string[]): string {
    let minConnections = Infinity;
    let selectedServer = servers[0];

    for (const server of servers) {
      const connections = parseInt((this.redisService.get(`connections:${server}`) || '0'));
  ...
  private weightedRoundRobin(servers: string[]): string {
    const weights = Array.from(this.servers.values());
    const totalWeight = weights.reduce((sum, weight) => sum + weight, 0);
    
    const key = 'load_balancer:weighted';
    let current = parseInt((this.redisService.get(key) || '0'));
    
    current = (current + 1) % totalWeight;
    this.redisService.setex(key, 3600, current.toString());
  ...
  async healthCheck(server: string): Promise<boolean> {
    ...
    const lastCheck = await this.redisService.get(key);
    ...
    try {
      ...
      if (isHealthy) {
        await this.redisService.setex(key, 300, Date.now().toString());
        return true;
      } else {
  ...
  async circuitBreaker(
    ...
    const failureCount = parseInt((this.redisService.get(circuitKey) || '0'));
    ...
    try {
      ...
      const newCount = await this.redisService.incr(circuitKey);
      if (newCount === 1) {
        await this.redisService.expire(circuitKey, Math.ceil(timeout / 1000));
      }
      throw error;
    }
  }
  ...
  async manageConnections(server: string, delta: number): Promise<void> {
    ...
    await this.redisService.incrby(key, delta);
